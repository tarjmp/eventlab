...
@foreach($messages as $message)
    ...
    <div id="msg-{{ $message->id }}"
         class="mt-2 p-2 rounded-lg msg-default @if($message->trashed()) msg-trashed @elseif($message->user->id == Auth::id()) msg-own @endif">
                ...
                <span class="float-right">
                    @if(\App\Tools\PermissionFactory::createDeleteMessage()->has($message->id ))
                        <span class="font-weight-bold" style="cursor:pointer;"
                              onclick="deleteChatMessage({{ $message->id }});" title="{{ __('chat.delete') }}">&nbsp;&times;&nbsp;</span>
                    @endif
                </span>
            @else
